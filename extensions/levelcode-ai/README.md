...
Native, Claude-first AI for LevelCode. The chat lives in its own activity-bar panel and talks
**directly** to the provider — there is no LevelCode server in between.
...
- **Ollama** — local models, no key. Point `levelcode.ai.ollama.url` at your server and set
  `levelcode.ai.ollama.model` to a model you've pulled.
