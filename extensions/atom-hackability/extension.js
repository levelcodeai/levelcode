/*---------------------------------------------------------------------------------------------
 *  LevelCode — Hackability (M3, feature 1: user init script)
 *
 *  Atom's signature "run my own code when the editor starts" hook. On startup we load
 *  ~/.levelcode/init.js and run it with a small `levelcode` convenience API plus the full
 *  `vscode` API in scope. It is live-reloadable ("LevelCode: Reload Init Script") — no restart.
 *
...
const INIT_DIR = path.join(os.homedir(), '.levelcode');
...
const TEMPLATE = `// LevelCode — User Init Script
// This file runs every time LevelCode starts. Edit it, then run
// "LevelCode: Reload Init Script" from the Command Palette (no restart needed).
//
// Two things are in scope:
//   • levelcode  — a small convenience API (everything you register is auto-cleaned on reload)
...
// levelcode.addStatusBarItem('🚀 init.js', 'Added from my init script');

// 2. A custom command (run it from the Command Palette):
// levelcode.registerCommand('myinit.hello', () => {
//   levelcode.showMessage('Hello from your init script!');
// });

// 3. React to saving a file:
// levelcode.onDidSave(doc => console.log('Saved:', doc.fileName));

// 4. Anything in the raw VS Code API:
// vscode.window.showInformationMessage('LevelCode init script loaded.');
`;

/** Build the friendly LevelCode API handed to the init script. Returns { api, subs }. */
function makeApi() {
...
function runInit(interactive) {
	if (!vscode.workspace.getConfiguration('levelcode').get('init.enabled', true)) { return false; }
	if (!fs.existsSync(INIT_PATH)) {
		if (interactive) { vscode.window.showInformationMessage('LevelCode: no init script yet. Use “LevelCode: Open Init Script” to create one.'); }
		return false;
	}
	...
	try {
		// Run the user's code with levelcode/vscode/require/console in scope.
		const fn = new Function('levelcode', 'vscode', 'require', 'console', '__dirname', '__filename', code);
		...
		if (interactive) { vscode.window.setStatusBarMessage('LevelCode init script reloaded', 2000); }
		return true;
	} catch (e) {
		const msg = (e && e.stack) ? e.stack : String(e);
		output.appendLine('[init] ERROR running init.js:\n' + msg);
		const pick = 'Show Log';
		vscode.window.showErrorMessage('LevelCode init script failed: ' + ((e && e.message) || e), pick)
...
async function openInit() {
	try {
		...
		vscode.window.showErrorMessage('LevelCode: could not create init script: ' + ((e && e.message) || e));
		return;
	}
...
function activate(context) {
	output = vscode.window.createOutputChannel('LevelCode Init');
	context.subscriptions.push(
		output,
		vscode.commands.registerCommand('levelcode.init.edit', openInit),
		vscode.commands.registerCommand('levelcode.init.reload', () => runInit(true)),
		{ dispose: disposeUser }
	);
