...
Built-in extension bringing Notepad++-style power editing to LevelCode.

**M1 feature 1 — Macros (this version):**

| Action | Default key | Command |
| --- | --- | --- |
| Start/stop recording | `Cmd+Shift+R` | `LevelCode: Macro: Start/Stop Recording` |
| Play last macro | `Cmd+Alt+P` | `LevelCode: Macro: Play Last` |
| Play last N times | — | `LevelCode: Macro: Play Last N Times…` |
| Save last macro | — | `LevelCode: Macro: Save Last Macro…` |
| Run saved macro | — | `LevelCode: Macro: Run Saved Macro…` |
| Delete saved macro | — | `LevelCode: Macro: Delete Saved Macro…` |
...
**Sublime-style persistence:** LevelCode ships with the strongest "hot exit" mode (`files.hotExit` = `onExitAndWindowClose`) as the **built-in default**, so unsaved edits and untitled tabs survive quitting the app and even an OS restart/shutdown — your work is continuously backed up to disk as you type. (`window.restoreWindows` is already `all` upstream.)
...
**Line operations:** right-click in the editor → **Line Operations**, or run any from the Command Palette (`LevelCode: Lines: …` / `LevelCode: Convert to …`). They act on the selected lines, or the whole file if nothing is selected.
...
**Column editor:** make a box selection (`Shift+Alt+drag`) or drop multiple cursors, then right-click → **Column Editor**, or run `LevelCode: Column: …` from the palette.
...
**Big-file mode:** large files open smoothly — the editor streams them into a memory-efficient buffer and reduces heavy features (syntax highlighting, etc.) automatically. LevelCode raises the open-without-confirmation limit (`workbench.editorLargeFileConfirmation` = 2048 MB) and shows a **⚡ Big-file mode** badge in the status bar with a one-time notice when a file is at least `levelcode.bigFile.thresholdMB` (default 256). Editing, scrolling, and find still work; turn the notice off with `levelcode.bigFile.notify`.

**AI features (temporarily off):** the upstream GitHub Copilot chat/agent UI is hidden by default (`chat.disableAIFeatures` = `true`) and the proprietary Copilot extension is not bundled. LevelCode's own AI layer (M2) will replace it; this default flips back on once our provider is wired in.
